...
RSpec.describe RegisteredApplicationsController, type: :controller do
  ...
  describe "POST create" do
    it "increases the number of registered_application by 1" do
      expect{ post :create, params: {user_id: user.id, registered_application1: {name: registered_application1.name, url: registered_application1.url} } }.to change(RegisteredApplication,:count).by(1)
    end

    it "increases the sum of registered_applications by 1" do
      count = user.registered_applications.count
      post :create, params: {user_id: user.id, registered_application_id: registered_application2.id }
  ...
  describe "DELETE destroy" do
    ...
    it "deletes the registered_application" do
      delete :destroy, params: { user_id: user.id, id: registered_application1.id }
    ...
    it "returns http success" do
      ...
      expect(flash[:notice]).to eq "\"#{registered_application1.name}\" was deleted successfully."
    end
  end
end
